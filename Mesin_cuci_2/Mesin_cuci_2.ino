...
const char* ssidList[] = { "POCO F4", "REDMI", "HELLO" }; 
const char* passList[] = { "", "REALME0885", "hello12345" };

const int totalNetworks = sizeof(ssidList) / sizeof(ssidList[0]);
...
void water_in_on(){
  ...
  while (test<=10){
    ...
    delay(1000);
    time_limit++;
    if (time_limit==30*60*1000){
      test=11;
      digitalWrite(water_in, HIGH);
      Serial.println("Waktu Mengisi Air Sudah Melebihi " + String(time_limit/60000) + " Menit, Proses di jeda");
      bot.sendMessage(CHAT_ID, "Waktu Mengisi Air Sudah Melebihi " + String(time_limit/60000) + " Menit, Proses di jeda", "");
      while(true){
...
void connectToWiFi() {
  ...
  while (WiFi.status() != WL_CONNECTED) {
    Serial.println("\nPercobaan koneksi WiFi: " + String(attempt++));

    for (int i = 0; i < totalNetworks; i++) {
      Serial.println("\n Mencoba konek ke SSID: " + String(ssidList[i]));
      ...
      if (WiFi.status() == WL_CONNECTED) {
        Serial.println("\n Berhasil konek ke " + String(ssidList[i]));
        break;
      }

      Serial.println("\n Gagal konek ke " + String(ssidList[i]));
      WiFi.disconnect(true);
      delay(1500);
    }

    if (WiFi.status() != WL_CONNECTED) {
      Serial.println("\n Semua SSID gagal dicoba. Mengulang dari awal...\n");
      delay(2000);
    }
  }
  ...
  bot.sendMessage(CHAT_ID, "Mesin Cuci Terkoneksi ke WiFi " + WiFi.SSID() + " dengan IP " + WiFi.localIP().toString(), "");
}
